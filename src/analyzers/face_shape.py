...
# ====================== 설정 상수들 ====================== #
# ML 모델 설정
ML_MODEL_PATH = "face_shape_rf_model_fixed.pkl"
ML_LABEL_ENCODER_PATH = "label_encoder_fixed.pkl"
...
def analyze_face_shape(
    ...
    debug: bool = False,
    use_ml: bool = True,  # ML 모델 우선 사용
) -> Dict:
    """
    이미지(BGR) 또는 landmarks로 얼굴형을 분석.
    ML 모델을 우선적으로 사용하고, 실패시 통계 기반으로 fallback
    반환: {"status","shape","confidence","top2","metrics", ("debug")}
    """

    # ML 모델 사용 시도 (이미지가 있을 때만)
    if use_ml and image_bgr is not None:
        logger.info("ML 모델로 얼굴형 분류 시도...")
        try:
            ml_shape, ml_confidence = classify_face_shape_ml(image_bgr)
            logger.info(f"ML 분류 결과: {ml_shape} (신뢰도: {ml_confidence:.2%})")

            # ML 결과 반환 (통계 기반과 동일한 형식으로)
            out = {
                "status": "ok",
                "shape": ml_shape,
                ...
                "method": "ML",  # 사용된 방법 표시
                "top2": [
                    {"label": ml_shape, "prob": ml_confidence},
                    {"label": "N/A", "prob": 0.0}  # ML은 단일 결과만 제공
                ],
            }
            if debug:
                out["debug"] = {"method": "ML", "model_path": ML_MODEL_PATH}
            return out

        except Exception as e:
            logger.warning(f"ML 모델 실행 실패, 통계 기반으로 fallback: {e}")

    # 통계 기반 분류 (기존 로직)
    logger.info("통계 기반 얼굴형 분류 사용...")
    ...
    logger.info(f"통계 기반 분류 결과: {res['shape']} (신뢰도: {res['confidence']:.2%})")

    out = {
        "status": "ok",
        "shape": res["shape"],
        "confidence": res["confidence"],
        "method": "Statistical",  # 사용된 방법 표시
        "top2": res["top2"],
        "metrics": m,
    }
    if debug:
        out["debug"] = {"landmarks_used": FACE_LANDMARKS, "probs": res["probs"], "confidence_level": res["confidence_level"], "method": "Statistical"}
    return out
...
def classify_face_shape_ml(image_bgr) -> Tuple[str, float]:
    ...
    # 모델 파일 존재 확인
    logger.info(f"모델 파일 확인: {model_path} (존재: {os.path.exists(model_path)})")
    logger.info(f"레이블 파일 확인: {label_path} (존재: {os.path.exists(label_path)})")
    ...
    try:
        ...
        import traceback
        logger.error(f"ML 모델 실행 실패: {e}")
        logger.error(f"상세 에러: {traceback.format_exc()}")
        logger.error("통계 방식으로 fallback")
        landmarks = detect_landmarks(image_bgr)
        if landmarks is None:
            return DEFAULT_FACE_SHAPE, DEFAULT_CONFIDENCE
        return classify_face_shape(landmarks)
